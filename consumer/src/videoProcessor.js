...
async function processVideo(item) {
    ...
    // Make sure the directories exist before FFmpeg runs.
    ...
    /*
     * Include uploadId so two different uploads with the same
     * filename don't overwrite each other.
     */
    const finalFilename =
        `${parsed.name}_${uploadId}.mp4`;

    const finalPath =
        path.join(UPLOAD_DIR, finalFilename);

    const previewFilename =
        `${parsed.name}_${uploadId}_preview.mp4`;

    const previewPath =
        path.join(PREVIEW_DIR, previewFilename);
    ...
    try {
        ...
        if (error.code !== "ENOENT") {
            console.error(
                `[Video Processor] Could not remove temporary file ${filePath}`,
                error
            );
        }
    }
    ...
    console.log(
        `[Video Processor] Finished ${filename}`
    );
